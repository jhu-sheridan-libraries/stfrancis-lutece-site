<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
            ...
            <xsl:choose>			
                <xsl:when test="not(string(display-portlet-title)='1')">
                    ...
                    <div class="portlet-content -lutece-border-radius-bottom">
                        <xsl:apply-templates select="enrollportlet-portlet" />
                    </div>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="enrollportlet-portlet">
        <xsl:apply-templates select="enrollportlet-portlet-content" />
    </xsl:template>

    <xsl:template match="enrollportlet-portlet-content">
